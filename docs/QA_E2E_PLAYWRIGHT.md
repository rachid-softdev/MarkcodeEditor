# QA end-to-end avec Playwright (MarkcodeEditor)
...
Start-Process -FilePath cmd -ArgumentList "/c","pnpm dev" 
  -RedirectStandardOutput "$env:TEMP\qa\MarkcodeEditor.dev.txt" 
  -RedirectStandardError "$env:TEMP\qa\MarkcodeEditor.err.txt" -WindowStyle Hidden
...
- GET / â†’ **HTTP 200**.
GET / renvoie HTTP 200 avec le titre MarkFlow - Editeur Markdown Collaboratif. Racine publique, aucun mur d auth sur /.
...
Aucun bypass d auth dev trouve dans markflow (app Next.js monolithique a la racine). Pages publiques testables ; pages protegees redirigent vers /login. Ajouter un bypass dev ou identifiants de seed pour les tester.
...
`
(racine du repo)/scripts/qa\        # (Ã  crÃ©er si campagne complÃ¨te)
